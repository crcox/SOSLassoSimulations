function [ ] = GenerateFigures_LASSO( varargin )
%UNTITLED3 Summary of this function goes here
%   Detailed explanation goes here
    p = inputParser();
    addParameter(p, 'AnnotatedData', '../../../../data/AnnotatedData.mat', @ischar);
    addParameter(p, 'PThreshold', 0.05, @isscalar);
    addParameter(p, 'FDRCorrect', true, @islogical);
    parse(p, varargin{:});
    
    alpha = p.Results.PThreshold;
    fdr = p.Results.FDRCorrect;
    
    load(p.Results.AnnotatedData,'AnnotatedData');
    final = LoadSimulationResults( 'final' );
    unitcodes_final = PairResultsWithUnitCodes(final,AnnotatedData);
    
    permutations = LoadSimulationResults( 'permutations' );
    baseprob = BaseProbability(permutations);
    unitcodes_baseprob = PairResultsWithUnitCodes(baseprob, AnnotatedData, ...
        'VariableToPair','baseprob','NewVariableName','baseprob');
    
    UnitCodesByCond = ApplyStatisticalTests(unitcodes_final, unitcodes_baseprob, 'AnalysisType', 'lasso');
    writetable(UnitCodesByCond, 'UnitCodesByCond.csv');
    MakeNetworkFigure(UnitCodesByCond,'PThreshold', alpha, 'FDRCorrect', fdr, 'AnalysisType', 'lasso');
    
    TPFP = TruePosFalsePos(UnitCodesByCond,'PThreshold', alpha, 'FDRCorrect', fdr);
    writetable(UnitCodesByCond, 'TPFP.csv');
    MakeSensitivityFigure(TPFP);
end

