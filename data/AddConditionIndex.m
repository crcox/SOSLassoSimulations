function [SortInfo,nvox] = AddConditionIndex(d,condition)
%% Setup
	subjects = categories(d.subject);
    example_id = categories(d.example_id);
    unit_id = categories(d.unit_id);
    nsub = numel(subjects);
	nitem = numel(example_id);
    z = d.subject == subjects{1} & d.example_id == example_id{1};
	N = cell2struct( ...
    ...
    SortInfo.condition = categorical();
